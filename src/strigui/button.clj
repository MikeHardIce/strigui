...
(defn- draw-hover
  ...
  [canvas btn]
  (when (not-empty btn)
    (apply b/box-border (conj [canvas :black 2] (:coord btn)))
    btn))
...
(defn- draw-clicked
  ...
  [canvas btn]
  (when (not-empty btn)
    (apply b/box-border (conj [canvas :green 2] (:coord btn)))
    btn))
...
(defmethod c2d/mouse-event ["main-window" :mouse-pressed] [event state]
  (let [context @wnd/context
        canvas (:canvas context)
        window (:window context)
        btn (first (filter #(wnd/within? (:coord %) (c2d/mouse-x window) (c2d/mouse-y window)) @b/boxes))]
    (when (not-empty btn)
